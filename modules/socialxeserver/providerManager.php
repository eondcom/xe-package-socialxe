...
class socialxeServerProviderManager{
	...
	function socialxeServerProviderManager(&$sessionManager, &$config){
		...
		// 제공하는 서비스
		$this->provider_list = array('twitter', 'me2day', 'facebook', 'yozm', 'google');
		...
		$this->provider['google'] = &socialxeServerProviderGoogle::getInstance($this->session, $config->google_client_id, $config->google_client_secret, $config->google_developer_key);
	}
